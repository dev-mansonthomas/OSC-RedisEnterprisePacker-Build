...
| **F-20** | 🟠 | **Append-only state orphans billable resources.** *Build fixed this on 2026-09-17 with `build_scripts/lib/env_file.sh` (sentinel-delimited blocks rewritten in place via temp file + `mv`, plus a warning when legacy duplicate assignments are detected). Port that library to Run — it is reusable as-is and covers both `osc-setup.sh`'s `OSC_*` block and `instanciate_image_outscale.sh`'s per-node IPs, though the latter also needs F-01's race fixed. Recorded as deliberate drift in `Build/scripts/shared-drift.baseline`.* Original finding:, same root cause as Build T-01: re-running `osc-setup.sh` creates a new Net and appends a second `OSC_*` block; `source` keeps the last, so the earlier Net/subnets/SG can never be torn down and keep billing. Also applies to `OUTSCALE_INSTANCE_PUBLIC_IP_<n>` across runs. | `osc/osc-setup.sh:213-227`; `instanciate_image_outscale.sh:220` | Rewrite a sentinel-delimited block in place; refuse to run when a live `OSC_NET_ID` exists unless `--force`. |
...
| **F-27** | ⚪ | **Adopt Build's `sleep 15` after VM termination.** Build's `tear_down_outscale.sh` waits 15 s between `DeleteVms` and deleting the default route; Run's copy does not. It gives Outscale time to release the NICs before the route table is torn down, and is the only executable difference between the two copies. Recorded in `Build/scripts/shared-drift.baseline`; adopting it makes the files identical again. | `osc/tear_down_outscale.sh:~97` | Add the `sleep 15`, then refresh Build's drift baseline. |
