...
| **T-10** | 🟡 | **Base OMI pinned by raw region-specific ID** (`ami-054f16b1`, eu-west-2). **Decision (2026-09-17): keep the explicit ID** — `source_omi_filter` is *not* the direction, because the roadmap is to publish **one OMI per region**, which means looping the build over regions with a different base OMI (and probably a different network setup) per region. An auto-resolving filter would hide exactly the mapping that needs to become explicit. Severity lowered 🟠→🟡: the risk is a future build break, not a current defect. | `packer/…pkr.hcl:36` | Restructure rather than auto-resolve: replace the scalar `source_omi` with a **`region → source_omi` map** in a variable (or a small `regions.auto.pkrvars.hcl`), and have the wrapper look up `OUTSCALE_REGION`. Fail with a clear message on an unmapped region. That is a one-entry map today and the seam for the multi-region loop later. Also record the resolved OMI ID in `manifest.json` for traceability, and add a comment giving the Ubuntu image name so the ID can be re-derived by hand. |
...
| **T-37** | 🟡 | **`packer` is not installed in the Colima VM**, so `packer fmt`/`validate`/`init` cannot run here — yet they are credential-free, exactly the posture of `tofu validate`. Add `packer` to `scripts/vm-provision.sh`, then `./03-vm-up.sh`. `oapi-cli` stays host-only by design. |
